...
const useStudyStore = create((set, get) => ({
  ...
  addTask: (taskData, selectedDate = null) => set((state) => {
    ...
    const newTask = {
      ...
      ...taskData,
      status: taskData.status || 'To Do', // Default status
  ...
  syncAddTask: async (taskData, selectedDate = null) => {
    try {
      ...
      const frontendTask = {
        ...
        quadrant: backendTask.quadrant,
        status: backendTask.status,
        due_date: backendTask.due_date, // Keep as YYYY-MM-DD string
        created_at: backendTask.created_at, // Keep as YYYY-MM-DD string
        updated_at: backendTask.created_at, // Backend doesn't have updated_at, use created_at
        task_date: dueDateStr || backendTask.created_at // Use provided dueDateStr or created_at
      };
      
      // Update store with converted task
      get().addTask(frontendTask, selectedDate);
      return frontendTask;
    } catch (error) {
  ...
  fetchPriorityMatrixTasks: async (dueDateStr = null) => {
    ...
    try {
      const backendTasks = await priorityMatrixApi.getTasks(null, dueDateStr);
      
      console.log('Backend tasks received:', backendTasks);
      console.log('Due date filter:', dueDateStr);
      ...
      const frontendTasks = backendTasks.map(task => ({
        ...
        status: task.status,
        due_date: task.due_date, // Keep as YYYY-MM-DD string
        created_at: task.created_at, // Keep as YYYY-MM-DD string
        updated_at: task.created_at, // Backend doesn't have updated_at, use created_at
        task_date: task.due_date || task.created_at // Use due_date or created_at as task_date
      }));
