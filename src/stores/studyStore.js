...
const useStudyStore = create((set, get) => ({
  ...
  syncAddTask: async (taskData, selectedDate = null) => {
    try {
      const backendTask = await priorityMatrixApi.createTask(taskData);
  ...
  // Fetch priority matrix tasks from backend
  fetchPriorityMatrixTasks: async () => {
    set({ isLoading: true, error: null });
    
    try {
      const tasks = await priorityMatrixApi.getTasks();
      set((state) => ({
        studyData: {
          ...state.studyData,
          eisenhower_matrix: {
